...
- [ ] 1. Убедиться, что merge выполняется только `dev -> main` (без прямых коммитов в `main`).
- [ ] 2. Обновить локальный `dev`: `git checkout dev && git pull --ff-only origin dev`.
- [ ] 3. Убедиться, что рабочее дерево чистое (кроме явно согласованных untracked файлов).
- [ ] 4. Повторно прогнать проверки: `ruff check .` и `pytest -q`.
- [ ] 5. Прогнать smoke текущего Telegram-бота (user-flow + admin-flow).
- [ ] 6. Прогнать smoke Mini App (`GET /miniapp`, auth, `Записаться`, `Мои заявки`, admin-блок).
- [ ] 7. Проверить feature flags для prod: `MINIAPP_DEV_LOGIN_ENABLED=false` (обязательно) и `MINIAPP_ENABLED` по выбранной стратегии релиза (см. раздел 4).
- [ ] 8. Проверить, что миграции актуальны и в `alembic` нет незакоммиченных изменений.
